...
import { CATEGORIES, PRODUCTS, type Product } from '@/lib/landingData';
    ...
    return (
        ...
        >
            ...
            <div className="mt-5 flex items-start justify-between gap-3">
                <div>
                    <h3 className="landing-display text-2xl leading-none">{p.name}</h3>
                    <p className="mt-1.5 text-sm opacity-70">{p.flavor}</p>
                </div>
                <span className="landing-display whitespace-nowrap text-xl">{p.price}</span>
            </div>

            <p className="mt-3 text-sm leading-relaxed opacity-65">{p.description}</p>
    ...
    return (
        <section className="px-5 pb-28 pt-32 sm:px-8 sm:pt-40">
            <div className="mx-auto max-w-[1280px]">
                {/* Header */}
                <div className="mb-10">
                    <Chip variant="yellow" size="md" className="mb-5">The Lineup</Chip>
                    <h1 className="landing-display text-[clamp(3rem,9vw,7rem)] leading-[0.88] text-white">
                        <TextReveal text="Every flavor, one crunch" />
                    </h1>
                    <p className="mt-5 max-w-lg font-grotesk text-base text-white/75">
                        The full Lay&apos;s lineup by Sarayo Alwadiya — cut thick, fried crisp, seasoned loud.
                    </p>
                </div>
                ...
                <div className="mb-10 flex flex-wrap gap-2.5">
                    {CATEGORIES.map((c) => (
                        <button key={c} onClick={() => setCat(c)} data-cursor="hover" aria-pressed={cat === c}>
                            <Chip variant={cat === c ? 'yellow' : 'outline'} size="md">{c}</Chip>
                        </button>
                    ))}
                </div>
            ...
            <div className="mx-auto mt-20 max-w-[1280px]">
                <div className="flex flex-col items-center justify-between gap-6 rounded-3xl border border-white/15 bg-brand-red-deep px-8 py-12 text-center sm:flex-row sm:text-left">
                    <div>
                        <h2 className="landing-display text-3xl text-white sm:text-4xl">Can&apos;t decide?</h2>
                        <p className="mt-2 font-grotesk text-sm text-white/70">Grab the whole lineup and settle it the loud way.</p>
                    </div>
