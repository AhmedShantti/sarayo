                    ...
                    >
                        ...
                        <div className="flex items-center justify-between border-b border-white/15 px-6 py-5">
                            <div className="flex items-center gap-3">
                                ...
                                <h2 className="landing-display text-2xl">Your Bag</h2>
                        ...
                        {done ? (
                            <div className="flex flex-1 flex-col items-center justify-center px-8 text-center">
                                ...
                                <h3 className="landing-display text-3xl">Order placed!</h3>
                                <p className="mt-3 max-w-xs font-grotesk text-sm text-white/75">
                                    Successfully checked out. Your crunch is on the way — thanks for snacking with Sarayo.
                                </p>
                                <button
                                    onClick={keepShopping}
                                    data-cursor="hover"
                            ...
                            <div className="flex flex-1 flex-col items-center justify-center px-8 text-center">
                                ...
                                <h3 className="landing-display text-3xl">Your bag is empty</h3>
                                <p className="mt-3 max-w-xs font-grotesk text-sm text-white/70">
                                    Nothing crunchy in here yet. Go pick a flavor or two.
                                </p>
                                <a
                                    href="/products"
                                    onClick={keepShopping}
                            ...
                            <>
                                        ...
                                        {items.map((l) => (
                                            ...
                                            >
                                                <span className="relative h-16 w-14 shrink-0 rounded-xl bg-[radial-gradient(circle_at_50%_40%,rgba(255,255,255,0.18),transparent_70%)]">
                                                    <Image src={l.src} alt={l.name} fill sizes="56px" className="object-contain p-1" />
                                                </span>
                                                <div className="min-w-0 flex-1">
                                                    <h4 className="landing-display text-lg leading-none">{l.name}</h4>
                                                    <p className="mt-1 text-xs text-white/60">{l.flavor}</p>
                                                    <div className="mt-2 inline-flex items-center gap-3 rounded-full border border-white/20 px-2 py-1">
                                                        <button onClick={() => dec(l.name)} data-cursor="hover" aria-label="Decrease" className="text-base leading-none text-white/80 hover:text-brand-yellow">−</button>
                                                        <span className="min-w-4 text-center font-grotesk text-sm font-semibold">{l.qty}</span>
                                                        <button onClick={() => inc(l.name)} data-cursor="hover" aria-label="Increase" className="text-base leading-none text-white/80 hover:text-brand-yellow">+</button>
                                                    </div>
                                                </div>
                                                <div className="flex flex-col items-end gap-2">
                                                    <span className="landing-display text-lg text-brand-yellow">{l.qty * l.price} EGP</span>
                                                    <button onClick={() => remove(l.name)} data-cursor="hover" className="text-[11px] uppercase tracking-wider text-white/50 hover:text-white">Remove</button>
                                                </div>
                                            </motion.div>
                                        ))}
                                    </AnimatePresence>
                                ...
                                <div className="border-t border-white/15 px-6 py-5">
                                    <div className="mb-4 flex items-baseline justify-between">
                                        <span className="font-grotesk text-sm uppercase tracking-wider text-white/70">Subtotal</span>
                                        <span className="landing-display text-3xl text-white">{subtotal} EGP</span>
                                    </div>
                                    <button
                                        onClick={checkout}
                                        data-cursor="hover"
                                    ...
                                    >
                                        ...
                                        Checkout · {subtotal} EGP
                                    </button>
                                    <p className="mt-3 text-center text-[11px] text-white/45">Free delivery over 150 EGP</p>
                                </div>
                            </>
                        )}
                    </motion.aside>
