...
// Clear a dead session and tell the dashboard to show the login screen.
// Called when an admin request 401s and the refresh token can't revive it.
function forceLogout() {
    logout();
    if (typeof window !== 'undefined') {
        window.dispatchEvent(new Event('sarayo-admin-logout'));
    }
}
...
async function authedFetch(path, options = {}, retry = true) {
    ...
    if (res.status === 401) {
        ...
        // Session is unrecoverable — clear it and bounce to the login screen.
        forceLogout();
    }
    return unwrap(res);
}
...
export async function getCategories() {
    // Public endpoint; returns active categories with id/name/slug/productCount.
...
export async function createCategory(payload) {
    return authedFetch('/admin/categories', {
        method: 'POST',
...
export async function updateCategory(id, payload) {
    return authedFetch(`/admin/categories/${id}`, {
        method: 'PATCH',
        body: JSON.stringify(payload),
    });
}
...
export async function deleteCategory(id) {
    return authedFetch(`/admin/categories/${id}`, {method: 'DELETE'});
}
